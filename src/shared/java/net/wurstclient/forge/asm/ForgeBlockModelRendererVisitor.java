...
public final class ForgeBlockModelRendererVisitor extends WurstClassVisitor {
	...
	public ForgeBlockModelRendererVisitor(ClassVisitor cv, boolean obf) {
		...
		String bufferBuilder = unmap("net/minecraft/client/renderer/" + (mcVersion.isLowerThan("1.12") ? "VertexBuffer" : "BufferBuilder"));
		
		String renderModelFlat_name = obf ? "c" : "renderModelFlat";
		String renderModelSmooth_name = obf ? "b" : "renderModelSmooth";
		String renderModel_desc = "(L" + iBlockAccess + ";" + "L" + iBakedModel + ";" + "L" + iBlockState + ";" + "L" + blockPos + ";" +
								   "L" + bufferBuilder + ";ZJ)Z";
		
		registerMethodVisitor(renderModelFlat_name, renderModel_desc, mv -> new RenderModelFlatVisitor(mv));
		registerMethodVisitor(renderModelSmooth_name, renderModel_desc, mv -> new RenderModelSmoothVisitor(mv));
	}
