...
public final class StateImplementationVisitor extends WurstClassVisitor {
	...
	public StateImplementationVisitor(ClassVisitor cv, boolean obf) {
		...
		registerMethodVisitor(getAmbientOcclusionLightValue_name, getAmbientOcclusionLightValue_desc,
							  mv -> new GetAmbientOcclusionLightValueVisitor(mv));
		registerMethodVisitor(isNormalCube_name, isNormalCube_desc, mv -> new IsNormalCubeVisitor(mv));
		registerMethodVisitor(shouldSideBeRendered_name, shouldSideBeRendered_desc, mv -> new ShouldSideBeRenderedVisitor(mv));
	}
	
	private static class GetAmbientOcclusionLightValueVisitor extends MethodVisitor {
		...
		public void visitInsn(int opcode) {
			if (opcode == Opcodes.FRETURN) {
				...
				mv.visitMethodInsn(Opcodes.INVOKESTATIC,
								   ...
								   "(FLnet/minecraft/block/state/IBlockState;)F",
								   false);
			}
	...
	private static class IsNormalCubeVisitor extends MethodVisitor {
		...
		public void visitInsn(int opcode) {
			if (opcode == Opcodes.IRETURN) {
				System.out.println("StateImplementationVisitor.IsNormalCubeVisitor.visitInsn()");
