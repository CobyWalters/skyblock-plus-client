...
public final class GuiInventoryVisitor extends WurstClassVisitor {
	...
	private class InitGuiVisitor extends MethodVisitor {
		...
		public void visitInsn(int opcode) {
			if (opcode == Opcodes.RETURN) {
				System.out.println("GuiInventoryVisitor.InitGuiVisitor.visitInsn()");
				
				mv.visitVarInsn(Opcodes.ALOAD, 0);
				mv.visitFieldInsn(Opcodes.GETFIELD, "net/minecraft/client/gui/inventory/GuiInventory", buttonList_name, "Ljava/util/List;");
	...
	private static class ActionPerformedVisitor extends MethodVisitor {
		...
		public void visitInsn(int opcode) {
			if(opcode == Opcodes.RETURN) {
				System.out.println("GuiInventoryVisitor.ActionPerformedVisitor.visitInsn()");
