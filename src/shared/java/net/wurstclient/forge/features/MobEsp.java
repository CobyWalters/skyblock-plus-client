...
public class MobEsp extends Feature {
	...
	private CheckboxSetting filterInvisible =
		new CheckboxSetting("Filter invisible", "Won't show invisible mobs.", false);
	
	private int mobBox;
	private ArrayList<EntityLiving> mobs = new ArrayList<>();
	
	public MobEsp() {
		super("MobESP", "Highlights nearby mobs.", false);
	...
	public void onUpdate(WUpdateEvent event) {
		...
		mobs.clear();
		Stream<EntityLiving> stream = 
			world.loadedEntityList.parallelStream()
	...
	private void renderTracers(double partialTicks) {
		...
		GL11.glBegin(GL11.GL_LINES);
		for(EntityLiving e : mobs) {
			Vec3d end = e.getEntityBoundingBox().getCenter().subtract(
								new Vec3d(e.posX, e.posY, e.posZ)
