...
public class AutoFarm extends Feature {
	...
	public void onUpdate(WUpdateEvent event) {
		if (wurst.getFeatures().freecam.isEnabled())
			return;
		if (wurst.getFeatures().bonemealAura.isEnabled() && wurst.getFeatures().bonemealAura.isGrowing())
			return;
		...
		isFarming = !blocksToReplant.isEmpty() || !blocksToHarvest.isEmpty();
	...
	public void onRenderWorldLast(RenderWorldLastEvent event) {
		...
		if(currentBlock != null) {
			...
			float p = prevProgress + (progress - prevProgress) * event.getPartialTicks();
			float red = p * 2F;
			float green = 2 - red;
	...
	private Stream<BlockPos> getBlockStream(BlockPos center, int range) {
		return StreamSupport.stream(BlockPos.getAllInBox(center.add(range, range, range),
	...
	private boolean canBeReplanted(BlockPos pos) {
		...
		if (item == Items.WHEAT_SEEDS || item == Items.CARROT || item == Items.POTATO || 
		    item == Items.BEETROOT_SEEDS || item == Items.PUMPKIN_SEEDS || item == Items.MELON_SEEDS)
