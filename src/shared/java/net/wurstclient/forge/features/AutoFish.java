...
public class AutoFish extends Feature {
	...
	private final SliderSetting validRange =
		new SliderSetting("Valid range",
						  "Any bites that occur outside of\n" +
						   "this range will be ignored.",
						  1.5, 0.25, 8, 0.25, ValueDisplay.DECIMAL);
	private final CheckboxSetting debugDraw =
		new CheckboxSetting("Debug draw",
							"Shows where bites are occurring\n" +
							 "and where they will be detected.",
							false);
	...
	public AutoFish() {
		super("AutoFish",
			  "Automatically catches fish using\n" +
			   "your best fishing rod.",
			  true);
	...
	public void onUpdate(WUpdateEvent event) {
		...
		// continue once every 15 ticks
		timer = (timer + 1) % 15;
		if (timer != 0)
			return;
		
		int bestRodSlot = getBestRodSlot(inventory);
		if (bestRodSlot == inventory.currentItem && player.fishEntity == null) {
			useRod();
		} else if (bestRodSlot == -1) {
			ChatUtils.message("Out of fishing rods.");
			setEnabled(false);
		} else if (bestRodSlot < 9) {
			inventory.currentItem = bestRodSlot;
		} else {
			moveBestRodToInventory(inventory, bestRodSlot);
		}
	}
	...
	private void updateRangeBox() {
		if (debugDraw.isChecked()) {
			GL11.glNewList(box, GL11.GL_COMPILE);
			AxisAlignedBB box = new AxisAlignedBB(-validRange.getValue(), -1 / 16.0, -validRange.getValue(),
												  validRange.getValue(), 1 / 16.0, validRange.getValue());
			GL11.glColor4f(1, 0, 0, 0.5F);
			GL11.glBegin(GL11.GL_LINES);
	...
	private int getBestRodSlot(InventoryPlayer inventory) {
		int bestRodValue = getRodValue(inventory.getStackInSlot(inventory.currentItem));
	...
	private int getRodValue(ItemStack stack) {
		...
		int luckOTSLvl = EnchantmentHelper.getEnchantmentLevel(Enchantments.LUCK_OF_THE_SEA, stack);
		int lureLvl = EnchantmentHelper.getEnchantmentLevel(Enchantments.LURE, stack);
		int unbreakingLvl = EnchantmentHelper.getEnchantmentLevel(Enchantments.UNBREAKING, stack);
		int mendingBonus = EnchantmentHelper.getEnchantmentLevel(Enchantments.MENDING, stack);
	...
	private void moveBestRodToInventory(InventoryPlayer inventory, int bestRodSlot) {
		int firstEmptySlot = inventory.getFirstEmptyStack();
		if (firstEmptySlot != -1) {
			if (firstEmptySlot >= 9)
				PlayerControllerUtils.windowClick_QUICK_MOVE(36 + inventory.currentItem);
			PlayerControllerUtils.windowClick_QUICK_MOVE(bestRodSlot);
		} else {
			PlayerControllerUtils.windowClick_PICKUP(bestRodSlot);
			PlayerControllerUtils.windowClick_PICKUP(36 + inventory.currentItem);
			timer = -bestRodSlot;
		}
	}
