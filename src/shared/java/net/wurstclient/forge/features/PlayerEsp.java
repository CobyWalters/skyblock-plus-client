...
public class PlayerEsp extends Feature {
	...
	private EnumSetting<Style> style = new EnumSetting<>("Style", Style.values(), Style.BOXES);
	private CheckboxSetting filterSleeping =
		new CheckboxSetting("Filter sleeping", "Won't show sleeping players.", false);
	private CheckboxSetting filterInvisible =
		new CheckboxSetting("Filter invisible", "Won't show invisible players.", false);
	
	private int playerBox;
	private ArrayList<EntityPlayer> players = new ArrayList<>();
	
	public PlayerEsp() {
		super("PlayerESP", "Highlights nearby players.", false);
	...
	public void onUpdate(WUpdateEvent event) {
		...
		players.clear();
		Stream<EntityPlayer> stream = 
			world.playerEntities.parallelStream()
	...
	@SubscribeEvent
	public void onCameraTransformViewBobbing(WCameraTransformViewBobbingEvent event) {
	...
	private void renderLines(double partialTicks) {
		Vec3d start =
			RotationUtils.getClientLookVec()
