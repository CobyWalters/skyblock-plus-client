...
public class Killaura extends Feature {
	...
	private EnumSetting<Priority> priority =
		new EnumSetting<>("Priority",
	...
	private CheckboxSetting filterPlayers =
		new CheckboxSetting("Filter players", "Won't attack other players.", false);
	private CheckboxSetting filterSleeping =
		new CheckboxSetting("Filter sleeping", "Won't attack sleeping players.", false);
	private SliderSetting filterFlying =
	...
	private CheckboxSetting filterMonsters =
		new CheckboxSetting("Filter monsters", "Won't attack zombies, creepers, etc.", false);
	private CheckboxSetting filterPigmen =
		new CheckboxSetting("Filter pigmen", "Won't attack zombie pigmen.", false);
	private CheckboxSetting filterEndermen =
		new CheckboxSetting("Filter endermen", "Won't attack endermen.", false);
	private CheckboxSetting filterAnimals =
		new CheckboxSetting("Filter animals", "Won't attack pigs, cows, etc.", false);
	private CheckboxSetting filterBabies =
		new CheckboxSetting("Filter babies", 
							"Won't attack baby pigs,\n" +
							 "baby villagers, etc.",
							false);
	private CheckboxSetting filterPets =
		new CheckboxSetting("Filter pets", "Won't attack tamed wolves,\n" + "tamed horses, etc.", false);
	private CheckboxSetting filterVillagers =
		new CheckboxSetting("Filter villagers", "Won't attack villagers.", false);
	private CheckboxSetting filterGolems =
		new CheckboxSetting("Filter golems",
							"Won't attack iron golems,\n" +
							 "snow golems and shulkers.",
							false);
	private CheckboxSetting filterInvisible =
		new CheckboxSetting("Filter invisible", "Won't attack invisible entities.", false);
	public CheckboxSetting swordSwitch =
		new CheckboxSetting("Sword auto switch",
							"Switches to the best sword\n" +
							 "on entity detection",
							false);
	private CheckboxSetting avoidSweeping =
		new CheckboxSetting("Avoid sweeping",
							"Attacks slightly faster and does\n" +
							 "less damage to avoid sweeping",
							false);
	
	private EntityLivingBase target;
	private Integer swordSlot;
	
	public Killaura() {
		super("Killaura", "Automatically attacks entities around you.", true);
		...
		addSetting(swordSwitch);
		addSetting(avoidSweeping);
	}
	...
	public void onUpdate(WUpdateEvent event) {
		...
		double delay = avoidSweeping.isChecked() ? 0.85: 1;
		...
		if (swordSwitch.isChecked()) {
			int swordSlotId = 0;
			if (swordSlot != null) {
				ItemStack swordSlotStack = WMinecraft.getPlayer().inventory.getStackInSlot(swordSlot);
				InventoryUtils.getId(swordSlotStack);
				//swordSlotId = swordSlotStack.getItem().getIdFromItem(swordSlotStack.getItem());
			}
			if (swordSlotId != 276) {
				swordSlot = null;
				for (int i = 0; i < 9; i++) {
					ItemStack stack = WMinecraft.getPlayer().inventory.getStackInSlot(i);
					int id = InventoryUtils.getId(stack);
			...
			if(swordSlot != null) {
				WMinecraft.getPlayer().inventory.currentItem = swordSlot;
			}
		} else {
			swordSlot = null;
		}
	...
	private enum Priority {
		...
		private String name;
		private Comparator<EntityLivingBase> comparator;
