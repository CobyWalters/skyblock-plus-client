...
public class ItemEsp extends Feature {
	...
	private CheckboxSetting names = new CheckboxSetting("Show item names", true);
	private EnumSetting<Style> style = new EnumSetting<>("Style", Style.values(), Style.BOXES);
	
	private ArrayList<EntityItem> items = new ArrayList<>();
	private int itemBox;
	
	public ItemEsp() {
		super("ItemESP", "Highlights nearby items.", false);
	...
	protected void onEnable() {
		...
		RenderUtils.drawOutlinedBox(new AxisAlignedBB(-0.175, 0, -0.175, 0.175, 0.35, 0.175));
		GL11.glEnd();
		GL11.glEndList();
		...
		for (EntityItem e : items) {
			...
			if (names.isChecked()) {
				...
				EntityRenderer.drawNameplate(WMinecraft.getFontRenderer(),
					...
					0, 1, 0, 0, mc.getRenderManager().playerViewY, mc.getRenderManager().playerViewX,
