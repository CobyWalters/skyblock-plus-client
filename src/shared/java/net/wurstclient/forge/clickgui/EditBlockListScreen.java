...
public final class EditBlockListScreen extends GuiScreen {
	...
	public void initGui() {
		...
		blockNameField = new GuiTextField(1, WMinecraft.getFontRenderer(), 64, height - 55, 150, 18);
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		drawCenteredString(WMinecraft.getFontRenderer(), blockList.getName() + " (" + listGui.getSize() + ")", width / 2, 12, 0xffffff);
		...
		if (blockNameField.getText().isEmpty() && !blockNameField.isFocused())
			drawString(WMinecraft.getFontRenderer(), "block name or ID", 68, height - 50, 0x808080);
	...
	private static class ListGui extends GuiScrollingList {
		...
		private String renderIconAndGetName(ItemStack stack, int y) {
			if (WItem.isNullOrEmpty(stack)) {
				...
				mc.getRenderItem().renderItemAndEffectIntoGUI(new ItemStack(Blocks.GRASS), 0, 0);
