...
public final class FeatureButton extends Component {
	...
	public void render(int mouseX, int mouseY, float partialTicks) {
		...
		if (feature.isEnabled())
			GL11.glColor4f(0, 1, 0, hFeature ? opacity * 1.5F : opacity);
		else
			GL11.glColor4f(bgColor[0], bgColor[1], bgColor[2], hFeature ? opacity * 1.5F : opacity);
		...
		int fx = x1 + ((settings ? getWidth() - 11 : getWidth()) - fr.getStringWidth(feature.getName())) / 2;
		int fy = y1 + 2;
	...
	public int getDefaultWidth() {
		int width =
			WMinecraft.getFontRenderer().getStringWidth(feature.getName()) + 2;
