...
public final class TextInputButton extends Component {
	...
	private final TextInputSetting setting;
	private int buttonWidth;
	
	public TextInputButton(TextInputSetting setting) {
	...
	public void handleMouseClick(int mouseX, int mouseY, int mouseButton) {
		...
		Minecraft.getMinecraft().displayGuiScreen(new TextInputScreen(Minecraft.getMinecraft().currentScreen, setting));
	}
	...
	public void render(int mouseX, int mouseY, float partialTicks) {
		...
		FontRenderer fr = WMinecraft.getFontRenderer();
		String text = setting.getName() + getPreviewText(setting.getText(), 30);
	...
	static public String getPreviewText(String text, int previewSize) {
		String[] words = text.split(" ");
		if (words[0].length() > previewSize - 3)
			return words[0].substring(0, previewSize - 3) + "\u00a7r...";
		String preview = "";
		for (int i = 0; i < words.length; ++i) {
			if (preview.length() + words[i].length() + 3 <= previewSize) {
				preview += words[i] + " ";
			} else {
			    preview = preview.trim() + "\u00a7r...";
			    break;
			}
		}
