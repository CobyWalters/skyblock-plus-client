...
public final class ClickGui {
	...
	public void init(FeatureList features) {
		...
		for (Feature feature : features.getRegistry())
			if (feature.getCategory() != null)
				windowMap.get(feature.getCategory()).add(new FeatureButton(feature));
	...
	private void handleScrollbarMouseClick(Window window, int mouseX, int mouseY, int mouseButton) {
		...
		int scrollbarY = (int) (outerHeight * (-window.getScrollOffset() / innerHeight) + 1);
	...
	private void handleComponentMouseClick(Window window, int mouseX, int mouseY, int mouseButton) {
		for (int i2 = window.countChildren() - 1; i2 >= 0; i2--) {
			...
			if (mouseX < c.getX() || mouseY < c.getY())
				continue;
			if (mouseX >= c.getX() + c.getWidth() || mouseY >= c.getY() + c.getHeight())
				continue;
	...
	public void render(int mouseX, int mouseY, float partialTicks) {
		...
		//boolean suppressSkyblock = ForgeWurst.getForgeWurst().getFeatureController().getServerName() == null;
		...
		if (dWheel != 0)
			for (int i = windows.size() - 1; i >= 0; i--) {
				...
				if (!window.isScrollingEnabled() || window.isMinimized() || window.isInvisible())
					continue;
	...
	private void renderWindow(Window window, int mouseX, int mouseY, float partialTicks) {
		...
		if (!window.isMinimized()) {
			...
			if (window.isScrollingEnabled()) {
				...
				// scrollbar
				GL11.glColor4f(acColor[0], acColor[1], acColor[2], hovering ? opacity * 1.5F : opacity);
				GL11.glBegin(GL11.GL_QUADS);
				GL11.glVertex2i(xs1, ys3);
				GL11.glVertex2i(xs1, ys4);
				GL11.glVertex2i(xs2, ys4);
