...
public final class EditItemListScreen extends GuiScreen {
	...
	public EditItemListScreen(GuiScreen prevScreen, ItemListSetting slider) {
	...
	@Override
	public void initGui() {
		listGui = new ListGui(mc, this, itemList.getItemNames());
		
		itemNameField = new GuiTextField(1, WMinecraft.getFontRenderer(), 64, height - 55, 150, 18);
		
		buttonList.add(addButton = new GuiButton(0, 214, height - 56, 30, 20, "Add"));
		buttonList.add(removeButton = new GuiButton(1, width - 150, height - 56, 100, 20, "Remove Selected"));
	...
	protected void actionPerformed(GuiButton button) throws IOException {
		...
		switch(button.id) {
			...
			case 1:
				itemList.remove(listGui.selected);
				break;
	...
	protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
		...
		if (mouseX < 50 || mouseX > width - 50 || mouseY < 32 || mouseY > height - 64)
			listGui.selected = -1;
	}
	...
	public void updateScreen() {
		...
		itemToAdd = Item.getByNameOrId(itemNameField.getText());
		addButton.enabled = itemToAdd != null;
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		drawCenteredString(WMinecraft.getFontRenderer(), itemList.getName() + " (" + listGui.getSize() + ")", width / 2, 12, 0xffffff);
		...
		if (itemNameField.getText().isEmpty() && !itemNameField.isFocused())
			drawString(WMinecraft.getFontRenderer(), "item name or ID", 68, height - 50, 0x808080);
		...
		listGui.renderIconAndGetName(new ItemStack(itemToAdd), height - 52);
	}
	...
	private static class ListGui extends GuiScrollingList {
		...
		public ListGui(Minecraft mc, EditItemListScreen screen, List<String> list) {
		...
		protected void drawSlot(int slotIdx, int entryRight, int slotTop, int slotBuffer, Tessellator tess) {
			...
			ItemStack stack = new ItemStack(Item.getByNameOrId(name));
		...
		private String renderIconAndGetName(ItemStack stack, int y) {
			if (WItem.isNullOrEmpty(stack)) {
				...
				mc.getRenderItem().renderItemAndEffectIntoGUI(new ItemStack(Items.MAP), 0, 0);
